...
module Intmapa = Map.Make(struct
  type t = int
  let compare = compare
end)
...
let misc_format_list sep f l =
  ignore 
    (List.fold_left
       (fun b x -> 
         format "%s" (if b then "" else sep);
         (f x);
         false)
       true l)
...
sig
  ...
  let mem c l = 
    List.exists (fun (c1,c2) -> c1 <= c && c <= c2) l 

  let ascii l =
    List.for_all (fun (c1,c2) -> c2 <= max_code) l
...
let rec format_t t0 =
  ...
  let rec format_list sep rnk l = match l with
    ...
    begin match t0.desc with
      ...
      | CSet cs ->
          let ns = CharSet.negate min_code max_code cs in (* verify this *)
          let p,l = if List.length ns < List.length cs then ("^",ns) else ("",cs) in
            wrap ("[" ^ p) "]" (fun () -> misc_format_list "" (fun pi -> msg "%s" (string_of_cset_code_pair pi)) l)
      | Rep(ti,i,jo) ->
...
let chars_of_charmap (m:charmap) : int list =
  let chars,_ =
    List.fold_right ( (* it could be fold_left, but then the representative of [a-be-f] would be "e" *)
      fun (c,di) (l,h) ->
...
let rec desc_charmap (t:t) =
  match t.desc with
    ...
    | Alt tl
    | Inter tl ->
        let ml = List.fold_left (fun ml ti -> get_charmap ti::ml) [] tl in
...
let match_code_list t0 l = 
  let t' = List.fold_left (fun ti ci -> ti.derivative ci) t0 l in 
  t'.final
...
let desc_hash d = 
  ...
  | Alt tl           -> 
      199 * List.fold_left (fun h ti -> h + 883 * ti.hash) 0 tl
  | Seq(t1,t2)       -> 
      821 * t1.hash + 919 * t2.hash
  | Inter tl         -> 
      71 * List.fold_left (fun h ti -> h + 883 * ti.hash) 0 tl
...
let desc_final = function
  ...
  | Alt tl      -> List.exists (fun ti -> ti.final) tl
  | Inter tl    -> List.for_all (fun ti -> ti.final) tl
...
(*   | Alt tl      -> 1 + List.fold_left (fun acc ti -> acc + ti.size) 0 tl *)
(*   | Inter tl    -> 1 + List.fold_left (fun acc ti -> acc + ti.size) 0 tl *)
...
let desc_ascii = function
  ...
  | Alt tl      -> List.for_all (fun t -> t.ascii) tl
  | Inter tl    -> List.exists (fun t -> t.ascii) tl
  | Diff(t1,t2) -> t1.ascii
...
(*       size = desc_size d0; *)
  ...
  let derivative_impl = 
    ...
    let res = match d0 with 
      ...
      | Alt tl -> 
          mk_table 
            (fun c -> mk_alts (List.map (fun ti -> ti.derivative c) tl))
      | Rep(t1,i,jo) -> 
          mk_table 
            (fun c -> 
               mk_seq 
      ...
      | Inter tl -> 
          mk_table 
            (fun c -> mk_inters (List.map (fun ti -> ti.derivative c) tl))
      | Diff(t1,t2) -> 
          mk_table 
...
and calc_reverse t = match t.desc with 
  ...
  | Alt tl        -> mk_alts (List.map get_reverse tl)
  | Rep(t1,i,jo)  -> mk_rep (get_reverse t1) i jo
  | Inter tl      -> mk_inters (List.map get_reverse tl)
...
and mk_cset cs = 
  let cs' = List.fold_left (fun l p -> CharSet.add p l) [] cs in 
    mk_gen_cset cs'
...
and mk_seqs tl = List.fold_left mk_seq epsilon tl
...
and mk_alts tl = List.fold_right mk_alt tl empty
...
and mk_inters tl = List.fold_left mk_inter anything tl
...
let string_of_char_codes l = 
  ...
  List.iter (fun ci -> Buffer.add_string buf (printable_string_of_char_code ci)) l;
  Buffer.contents buf

(* a printable version of the representative (do not use internally unless it is to print something) *)
let representative t0 = 
  map_option string_of_char_codes (get_representative t0)
...
let rec mk_expand t0 c t = match t0.desc with
  ...
  | Alt tl -> 
      mk_alts (List.map (fun ti -> mk_expand ti c t) tl)
  | Rep(t1,i,jo) -> 
      mk_rep (mk_expand t1 c t) i jo
  | Inter tl -> 
      mk_inters (List.map (fun ti -> mk_expand ti c t) tl)
  | Diff(t1,t2) -> 
          ...
          if Queue.is_empty q then None
            ...
            | C(l1,l2,l3,t2i,t2j) ->
                if t2i.final && t2j.final then
                  let w1 = string_of_char_codes (List.rev l1) in
                  let w2 = string_of_char_codes (List.rev l2) in
                  let w3 = string_of_char_codes (List.rev l3) in
...
let match_string_positions t0 w = 
  ...
  let rec loop acc i ti = 
    let acc' = 
      if ti.final then IntSet.add i acc 
      else acc in 
      if i=n then acc'
      else loop acc' (succ i) (ti.derivative (Char.code w.[i])) in 
    loop IntSet.empty 0 t0

let match_string_reverse_positions t0 w = 
  let n = String.length w in 
  let rec loop acc i ti = 
    let acc' = 
      if ti.final then IntSet.add (succ i) acc 
      else acc in 
      if i < 0 then acc'
...
let is_singleton s0 = 
  let to_csets rep = List.map (fun c -> mk_cset[(c,c)]) rep in
...
let seq_split s1 s2 w =
  let ps = split_positions s1 s2 w in 
    if not (IntSet.cardinal ps = 1) then 
      None
    else
      let n = String.length w in 
      let j = IntSet.choose ps in 
      let s1,s2 = (String.sub w 0 j, String.sub w j (n-j)) in 
      Some (s1,s2)
          
let star_split s1 w = 
  let s1_star = mk_star s1 in 
  let ps = IntSet.remove 0 (split_positions s1_star s1_star w) in 
  let _,rev = 
    IntSet.fold 
